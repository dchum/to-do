...
DeclareCDKWidgets(ENTRY, Entry, setCdk, String);
                      ...
                      boolean shadow)
{
   ...
   if ((entry = newCDKWidget(CDKENTRY, &my_funcs)) == NULL)
      return (NULL);
   ...
   registerCDKWidget(cdkscreen, vENTRY, entry);
...
char *activateCDKEntry(CDKENTRY *entry, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      for (;;)
      {
         chtype input = (chtype)getchCDKWidget(ObjOf(entry), &functionKey);
         // fprintf(stderr, "%d\n", *actions);  
...
static int _injectCDKEntry(CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind(vENTRY, widget, input) != 0)
      {
...
static void _destroyCDKEntry(CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings(vENTRY, entry);

      /* Unregister this object. */
      unregisterCDKWidget(vENTRY, entry);
   }
}
