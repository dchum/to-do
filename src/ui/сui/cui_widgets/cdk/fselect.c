...
DeclareCDKWidgets (FSELECT, Fselect, _setMy, String);
			   ...
			   boolean shadow)
{
   ...
   if ((fselect = newCDKWidget (CDKFSELECT, &my_funcs)) == NULL)
        return (NULL);
   ...
   for (x = 0; x < (int)SIZEOF (bindings); ++x)
      bindCDKWidget (vFSELECT,
		     fselect,
		     (chtype)bindings[x].from,
		     getcCDKBind,
		     (void *)(long)bindings[x].to);

   registerCDKWidget (cdkscreen, vFSELECT, fselect);
...
char *activateCDKFselect (CDKFSELECT *fselect, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      for (;;)
      {
	 chtype input = (chtype)getchCDKWidget (ObjOf (fselect->entryField), &functionKey);
...
static void _destroyCDKFselect (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vFSELECT, fselect);
      ...
      unregisterCDKWidget (vFSELECT, fselect);
   }
}
 ...
 */
static int displayFileInfoCB (EWidgetType objectType GCC_UNUSED,
			      void *object,
			      void *clientData,
			      chtype key GCC_UNUSED)
{
   ...
   getchCDKWidget (ObjOf (infoLabel), &functionKey);
 ...
 */
static int completeFilenameCB (EWidgetType objectType GCC_UNUSED,
 ...
 */
void deleteFileCB (EWidgetType objectType GCC_UNUSED, void *object, void *clientData)
{
 ...
 */
static int fselectAdjustScrollCB (EWidgetType objectType GCC_UNUSED,
				  void *object GCC_UNUSED,
				  void *clientData,
				  chtype key)
{
