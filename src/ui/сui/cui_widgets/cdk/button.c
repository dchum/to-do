...
DeclareCDKWidgets (BUTTON, Button, setCdk, Int);
			 ...
			 boolean shadow)
{
   ...
   if ((button = newCDKWidget (CDKBUTTON, &my_funcs)) == NULL)
        return (NULL);
   ...
   /* Register this baby. */
   registerCDKWidget (cdkscreen, vBUTTON, button);
...
int activateCDKButton (CDKBUTTON *button, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      for (;;)
      {
	 chtype input = (chtype)getchCDKWidget (ObjOf (button), &functionKey);
...
void positionCDKButton (CDKBUTTON *button)
{
   ...
   while (key != KEY_ENTER)
   {
      key = (chtype)getchCDKWidget (ObjOf (button), &functionKey);
      if (key == KEY_UP || key == '8')
      {
...
static void _destroyCDKButton (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vBUTTON, button);

      /* Unregister the object. */
      unregisterCDKWidget (vBUTTON, button);
   }
}
...
static int _injectCDKButton (CDKOBJS *object, chtype input)
{
   ...
   if (checkCDKWidgetBind (vBUTTON, widget, input) != 0)
   {
