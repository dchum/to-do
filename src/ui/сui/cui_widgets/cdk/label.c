...
DeclareCDKWidgets (LABEL, Label, setCdk, Unknown);
		       ...
		       boolean shadow)
{
   ...
   if (rows <= 0
       || (label = newCDKWidget (CDKLABEL, &my_funcs)) == NULL
   ...
   /* Register this baby. */
   registerCDKWidget (cdkscreen, vLABEL, label);
...
static void _destroyCDKLabel (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vLABEL, label);

      /* Unregister the object. */
      unregisterCDKWidget (vLABEL, label);
   }
}
...
char waitCDKLabel (CDKLABEL *label, char key)
{
   ...
   if (key == 0)
   {
      code = getchCDKWidget (ObjOf (label), &functionKey);
   }
   else
   {
