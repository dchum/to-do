...
DeclareCDKWidgets (TEMPLATE, Template, setCdk, String);
			     ...
			     boolean shadow)
{
   ...
   if (plate == NULL
       || (cdktemplate = newCDKWidget (CDKTEMPLATE, &my_funcs)) == NULL)
        return (NULL);
   ...
   registerCDKWidget (cdkscreen, vTEMPLATE, cdktemplate);
...
char *activateCDKTemplate (CDKTEMPLATE *cdktemplate, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      for (;;)
      {
	 chtype input = (chtype)getchCDKWidget (ObjOf (cdktemplate), &functionKey);
...
static int _injectCDKTemplate (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vTEMPLATE, widget, input) != 0)
      {
...
static void _destroyCDKTemplate (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vTEMPLATE, cdktemplate);

      unregisterCDKWidget (vTEMPLATE, cdktemplate);
   }
}
