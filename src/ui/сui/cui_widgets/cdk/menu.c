...
DeclareCDKWidgets (MENU, Menu, setCdk, Int);
		     ...
		     chtype subtitleAttr)
{
   ...
   if ((menu = newCDKWidget (CDKMENU, &my_funcs)) == NULL)
        return (NULL);
   ...
   /* Register this baby. */
   registerCDKWidget (cdkscreen, vMENU, menu);
...
int activateCDKMenu (CDKMENU *menu, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      ...
      for (;;)
      {
	 chtype input = (chtype)getchCDKWidget (ObjOf (menu), &functionKey);
...
static int _injectCDKMenu (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vMENU, widget, input) != 0)
      {
...
static void _destroyCDKMenu (CDKOBJS *object)
{
   if (object != NULL)
   {
	 ...
	 for (y = 0; y < menu->subsize[x]; y++)
	 {
      ...
      cleanCDKWidgetBindings (vMENU, menu);

      /* Unregister this object. */
      unregisterCDKWidget (vMENU, menu);
   }
}
