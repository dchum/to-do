...
DeclareCDKWidgets (SELECTION, Selection, setCdk, Int);
			       ...
			       boolean shadow)
{
   ...
   /* Register this baby. */
   registerCDKWidget (cdkscreen, vSELECTION, selection);
...
int activateCDKSelection (CDKSELECTION *selection, chtype *actions)
{
   ...
   if (actions == NULL)
   {
	 ...
	 input = (chtype)getchCDKWidget (ObjOf (selection), &functionKey);
...
static int _injectCDKSelection (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vSELECTION, widget, input) != 0)
      {
...
static void _destroyCDKSelection (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vSELECTION, selection);

      /* Unregister this object. */
      unregisterCDKWidget (vSELECTION, selection);
   }
}
