...
DeclareCDKWidgets (MATRIX, Matrix, setCdk, Int);
			 ...
			 boolean shadow)
{
   ...
   if ((matrix = newCDKWidget (CDKMATRIX, &my_funcs)) == NULL)
   {
      return (NULL);
   }
   ...
   for (x = 0; x < (int)SIZEOF (bindings); ++x)
      bindCDKWidget (vMATRIX, matrix,
		     (chtype)bindings[x].from,
		     getcCDKBind,
		     (void *)(long)bindings[x].to);

   /* Register this baby. */
   registerCDKWidget (cdkscreen, vMATRIX, matrix);
...
int activateCDKMatrix (CDKMATRIX *matrix, chtype *actions)
{
   ...
   if (actions == NULL)
   {
	 ...
	 input = (chtype)getchCDKWidget (ObjOf (matrix), &functionKey);
...
static int _injectCDKMatrix (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vMATRIX, widget, input) != 0)
      {
...
static void _destroyCDKMatrix (CDKOBJS *object)
{
   if (object != NULL)
   {
	 ...
	 for (y = 1; y <= matrix->vcols; y++)
	 {
      ...
      cleanCDKWidgetBindings (vMATRIX, matrix);

      /* Unregister this object. */
      unregisterCDKWidget (vMATRIX, matrix);
   }
}
