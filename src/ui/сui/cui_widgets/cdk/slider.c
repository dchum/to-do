...
DeclareCDKWidgets(SLIDER, Slider, setCdk, Int);
                        ...
                        boolean shadow)
{
   ...
   if ((widget = newCDKWidget(CDKSLIDER, &my_funcs)) == NULL)
      return (NULL);
   ...
   for (x = 0; x < (int)SIZEOF(bindings); ++x)
      bindCDKWidget(vSLIDER,
                    widget,
                    (chtype)bindings[x].from,
                    getcCDKBind,
                    (void *)(long)bindings[x].to);

   registerCDKWidget(cdkscreen, vSLIDER, widget);
...
int activateCDKSlider(CDKSLIDER *widget, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      ...
      for (;;)
      {
         chtype input = (chtype)getchCDKWidget(ObjOf(widget), &functionKey);
   ...
   else
   {
      ...
      for (x = 0; x < length; x++)
      {
         ret = (int)injectCDKSlider(widget, actions[x]);
         if (widget->exitType != vEARLY_EXIT)
         {
            return ret;
         }
...
#define SCANF_FMT "%d%c"
   ...
   if (temp != NULL)
   {
      ...
      if (modify && sscanf(temp, SCANF_FMT, &value, &test) == 2 && test == ' ' && value >= widget->low && value <= widget->high)
      {
         setCDKSliderValue(widget, (int)value);
         result = TRUE;
      }
      free(data);
   }
...
#define Decrement(value, by) \
   if (value - by < value)   \
   value -= by
#define Increment(value, by) \
   if (value + by > value)   \
   value += by
...
static int _injectCDKSlider(CDKOBJS *object, chtype input)
{
   ...
   if (PreProcessFuncOf(widget) != NULL)
   {
      ...
      ppReturn = PreProcessFuncOf(widget)(vSLIDER,
                                          widget,
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind(vSLIDER, widget, input) != 0)
      {
      ...
      {
         PostProcessFuncOf(widget)(vSLIDER,
                                   widget,
 ...
 */
static void _moveCDKSlider(CDKOBJS *object,
                           int xplace,
                           int yplace,
                           boolean relative,
...
static void _destroyCDKSlider(CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings(vSLIDER, widget);

      /* Unregister this object. */
      unregisterCDKWidget(vSLIDER, widget);
   }
}
 ...
 */
void setCDKSlider(CDKSLIDER *widget,
                  int low,
                  int high,
                  int value,
                  boolean Box)
