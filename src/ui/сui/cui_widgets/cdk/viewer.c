...
DeclareCDKWidgets (VIEWER, Viewer, setCdk, Unknown);
			 ...
			 boolean shadow)
{
   ...
   if ((viewer = newCDKWidget (CDKVIEWER, &my_funcs)) == NULL)
        return (NULL);
   ...
   for (x = 0; x < (int)SIZEOF (bindings); ++x)
      bindCDKWidget (vVIEWER,
		     viewer,
		     (chtype)bindings[x].from,
		     getcCDKBind,
		     (void *)(long)bindings[x].to);

   registerCDKWidget (cdkscreen, vVIEWER, viewer);
...
int activateCDKViewer (CDKVIEWER *widget, chtype *actions GCC_UNUSED)
{
   ...
   for (;;)
   {
      ...
      if (!checkCDKWidgetBind (vVIEWER, widget, input))
      {
	 switch (input)
	 {
	 case KEY_TAB:
...
static void popUpLabel (CDKVIEWER *viewer, CDK_CSTRING2 mesg)
{
   ...
   getchCDKWidget (ObjOf (label), &functionKey);
...
static void _destroyCDKViewer (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vVIEWER, viewer);

      /* Unregister this object. */
      unregisterCDKWidget (vVIEWER, viewer);
   }
}
