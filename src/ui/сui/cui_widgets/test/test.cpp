...
int main( )
{
    ...
    // int height_, width_;
    // getmaxyx(screen.get()->window, height_, width_);

    // std::cout << height_ << " " << width_ << std::endl;
    // std::cout << LINES << " " << COLS << endl;
...
//     cout << wdgt_box->width() << " " << wdgt_box->height() << endl;
...
//   //  t->CurrentValue();
    ...
    // CUIRadio* rd = new CUIRadio (wdgt_box, 60, 10, 25, 25, radio, list, list.count(), 35, 9001, 262144, 1, 1, 0);
    // cout << rd->width() << " " << rd->height() << endl;
    // wdgt_box->draw();
