...
DeclareCDKWidgets (SWINDOW, Swindow, setCdk, Int);
			   ...
			   boolean shadow)
{
   ...
   if ((swindow = newCDKWidget (CDKSWINDOW, &my_funcs)) == NULL)
        return (NULL);
   ...
   /* Register this baby. */
   registerCDKWidget (cdkscreen, vSWINDOW, swindow);
...
void activateCDKSwindow (CDKSWINDOW *swindow, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      ...
      for (;;)
      {
	 chtype input = (chtype)getchCDKWidget (ObjOf (swindow), &functionKey);
...
static int _injectCDKSwindow (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vSWINDOW, widget, input) != 0)
      {
...
static void _destroyCDKSwindow (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      /* Clean the key bindings. */
      cleanCDKWidgetBindings (vSWINDOW, swindow);

      /* Unregister this object. */
      unregisterCDKWidget (vSWINDOW, swindow);
   }
}
