...
DeclareCDKWidgets (RADIO, Radio, setCdk, Int);
		       ...
		       boolean shadow)
{
   ...
   if ((radio = newCDKWidget (CDKRADIO, &my_funcs)) == NULL)
   {
      return (NULL);
   }
   ...
   /* Register this baby. */
   registerCDKWidget (cdkscreen, vRADIO, radio);
...
int activateCDKRadio (CDKRADIO *radio, chtype *actions)
{
   ...
   if (actions == NULL)
   {
	 ...
	 input = (chtype)getchCDKWidget (ObjOf (radio), &functionKey);
...
static int _injectCDKRadio (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vRADIO, widget, input) != 0)
      {
...
static void _destroyCDKRadio (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vRADIO, radio);

      /* Unregister this object. */
      unregisterCDKWidget (vRADIO, radio);
   }
}
