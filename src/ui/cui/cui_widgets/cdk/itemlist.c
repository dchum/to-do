...
DeclareCDKWidgets (ITEMLIST, Itemlist, setCdk, Int);
			     ...
			     boolean shadow)
{
   ...
   if ((itemlist = newCDKWidget (CDKITEMLIST, &my_funcs)) == NULL
       || !createList (itemlist, item, count))
   {
      destroyCDKWidget (itemlist);
      return (NULL);
   }
   ...
   /* Register this baby.                */
   registerCDKWidget (cdkscreen, vITEMLIST, itemlist);
...
int activateCDKItemlist (CDKITEMLIST *itemlist, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      ...
      for (;;)
      {
	 chtype input = (chtype)getchCDKWidget (ObjOf (itemlist), &functionKey);
...
static int _injectCDKItemlist (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vITEMLIST, widget, input) != 0)
      {
...
static void _destroyCDKItemlist (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vITEMLIST, itemlist);

      /* Unregister this object. */
      unregisterCDKWidget (vITEMLIST, itemlist);
   }
}
