...
DeclareCDKWidgets (MENTRY, Mentry, setCdk, String);
			 ...
			 boolean shadow)
{
   ...
   if ((mentry = newCDKWidget (CDKMENTRY, &my_funcs)) == NULL)
        return (NULL);
   ...
   /* Register this baby. */
   registerCDKWidget (cdkscreen, vMENTRY, mentry);
...
char *activateCDKMentry (CDKMENTRY *mentry, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      for (;;)
      {
	 chtype input = (chtype)getchCDKWidget (ObjOf (mentry), &functionKey);
...
static int _injectCDKMentry (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vMENTRY, widget, input) != 0)
      {
...
static void _destroyCDKMentry (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vMENTRY, mentry);

      /* Unregister this object. */
      unregisterCDKWidget (vMENTRY, mentry);
   }
}
