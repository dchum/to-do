...
DeclareCDKWidgets (SCROLL, Scroll, setCdk, Int);
			 ...
			 boolean shadow)
{
   ...
   if ((scrollp = newCDKWidget (CDKSCROLL, &my_funcs)) == NULL)
   {
      destroyCDKWidget (scrollp);
      return (NULL);
   }
   ...
   for (x = 0; x < (int)SIZEOF (bindings); ++x)
      bindCDKWidget (vSCROLL,
		     scrollp,
		     (chtype)bindings[x].from,
		     getcCDKBind,
		     (void *)(long)bindings[x].to);

   registerCDKWidget (cdkscreen, vSCROLL, scrollp);
...
int activateCDKScroll (CDKSCROLL *scrollp, chtype *actions)
{
   ...
   if (actions == NULL)
   {
	 ...
	 input = (chtype)getchCDKWidget (ObjOf (scrollp), &functionKey);
...
static int _injectCDKScroll (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vSCROLL, widget, input) != 0)
      {
...
static void _destroyCDKScroll (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vSCROLL, scrollp);

      /* Unregister this object. */
      unregisterCDKWidget (vSCROLL, scrollp);
   }
}
			      ...
			      const char *value)
{
   if (number > 0)
   {
	 ...
	 *used = ((need + 2) * 2);
	 if (*work == NULL)
	 {
	    if ((*work = (char *)malloc (*used)) == NULL)
	       return FALSE;
	 }
	 else
	 {
	    if ((*work = (char *)realloc (*work, *used)) == NULL)
	       return FALSE;
	 }
      }
