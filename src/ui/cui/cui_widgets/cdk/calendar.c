...
DeclareCDKWidgets (CALENDAR, Calendar, setCdk, Int);
			     ...
			     boolean shadow)
{
   ...
   if ((calendar = newCDKWidget (CDKCALENDAR, &my_funcs)) == NULL)
        return (NULL);
   ...
   for (x = 0; x < (int)SIZEOF (bindings); ++x)
      bindCDKWidget (vCALENDAR,
		     calendar,
		     (chtype)bindings[x].from,
		     getcCDKBind,
		     (void *)(long)bindings[x].to);

   registerCDKWidget (cdkscreen, vCALENDAR, calendar);
...
time_t activateCDKCalendar (CDKCALENDAR *calendar, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      for (;;)
      {
	 chtype input = (chtype)getchCDKWidget (ObjOf (calendar), &functionKey);
...
static int _injectCDKCalendar (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vCALENDAR, widget, input) != 0)
      {
...
static void _destroyCDKCalendar (CDKOBJS *object)
{
   if (object != NULL)
   {
	 ...
	 freeChar (calendar->MonthName[x]);
      ...
      cleanCDKWidgetBindings (vCALENDAR, calendar);

      /* Unregister the object. */
      unregisterCDKWidget (vCALENDAR, calendar);
   }
}
