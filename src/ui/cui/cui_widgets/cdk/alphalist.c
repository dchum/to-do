...
static int preProcessEntryField (EWidgetType, void *, void *, chtype);
static int createList (CDKALPHALIST *alphalist, CDK_CSTRING *list, int listSize);

DeclareSetXXchar (static, _setMy);
DeclareCDKWidgets (ALPHALIST, Alphalist, _setMy, String);
			       ...
			       boolean shadow)
{
   ...
   if ((alphalist = newCDKWidget (CDKALPHALIST, &my_funcs)) == NULL
       || !createList (alphalist, list, listSize))
   {
      destroyCDKWidget (alphalist);
      return (NULL);
   }
   ...
   for (x = 0; x < (int)SIZEOF (bindings); ++x)
      bindCDKWidget (vALPHALIST,
		     alphalist,
		     (chtype)bindings[x].from,
		     getcCDKBind,
		     (void *)(long)bindings[x].to);

   registerCDKWidget (cdkscreen, vALPHALIST, alphalist);
...
static void _destroyCDKAlphalist (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vALPHALIST, alphalist);
      ...
      unregisterCDKWidget (vALPHALIST, alphalist);
   }
}
 ...
 */
static int adjustAlphalistCB (EWidgetType objectType GCC_UNUSED, void
			      *object GCC_UNUSED,
			      void *clientData,
			      chtype key)
{
 ...
 */
static int preProcessEntryField (EWidgetType cdktype GCC_UNUSED, void
				 *object GCC_UNUSED,
				 void *clientData,
				 chtype input)
{
   ...
   else if (isCDKWidgetBind (ObjTypeOf (alphalist), ObjOf (alphalist), input))
   {
 ...
 */
static int completeWordCB (EWidgetType objectType GCC_UNUSED, void *object GCC_UNUSED,
			   void *clientData,
