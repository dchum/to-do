...
DeclareCDKWidgets (BUTTONBOX, Buttonbox, setCdk, Int);
			       ...
			       boolean shadow)
{
   ...
   if (buttonCount <= 0
       || (buttonbox = newCDKWidget (CDKBUTTONBOX, &my_funcs)) == NULL
   ...
   /* Register this baby. */
   registerCDKWidget (cdkscreen, vBUTTONBOX, buttonbox);
...
int activateCDKButtonbox (CDKBUTTONBOX *buttonbox, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      for (;;)
      {
	 chtype input = (chtype)getchCDKWidget (ObjOf (buttonbox), &functionKey);
...
static int _injectCDKButtonbox (CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind (vBUTTONBOX, widget, input) != 0)
      {
...
static void _destroyCDKButtonbox (CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings (vBUTTONBOX, buttonbox);

      /* Unregister this object. */
      unregisterCDKWidget (vBUTTONBOX, buttonbox);
   }
}
