...
DeclareCDKWidgets(DIALOG, Dialog, setCdk, Int);
                        ...
                        boolean shadow)
{
   ...
   /* Register this baby. */
   registerCDKWidget(cdkscreen, vDIALOG, dialog);
...
int activateCDKDialog(CDKDIALOG *dialog, chtype *actions)
{
   ...
   if (actions == NULL)
   {
      for (;;)
      {
         chtype input = (chtype)getchCDKWidget(ObjOf(dialog), &functionKey);
...
static int _injectCDKDialog(CDKOBJS *object, chtype input)
{
   ...
   if (ppReturn != 0)
   {
      ...
      if (checkCDKWidgetBind(vDIALOG, widget, input) != 0)
      {
...
static void _destroyCDKDialog(CDKOBJS *object)
{
   if (object != NULL)
   {
      ...
      cleanCDKWidgetBindings(vDIALOG, dialog);

      /* Unregister this object. */
      unregisterCDKWidget(vDIALOG, dialog);
   }
}
